...
const PaymentScreenshotSection = ({ order }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (s?.screenshotUrl && s?.status === 'PENDING') setUploaded(true);
    }).catch(() => {});
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploaded(true);
      setPayStatus((p) => ({ ...p, status: 'PENDING', screenshotUrl: preview }));
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm mt-4">
      <h2 className="font-bold text-gray-900 text-sm mb-3">💳 EasyPaisa Payment</h2>
      ...
      {status === 'COMPLETED' && (
        <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-green-600">Your payment has been confirmed.</p>
          </div>
        </div>
      )}
      ...
      {status === 'FAILED' && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-3">
          ...
          {payStatus?.adminNote && <p className="text-xs text-red-500 mt-0.5">{payStatus.adminNote}</p>}
          <p className="text-xs text-gray-500 mt-1">Please re-upload your screenshot below.</p>
        </div>
      )}

      {/* EasyPaisa number — show if not verified */}
      {status !== 'COMPLETED' && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 mb-3">
          ...
          <p className="text-xl font-extrabold text-gray-900">{epInfo?.number || '03008356059'}</p>
          <p className="text-xs text-gray-500">{epInfo?.accountName || 'ZOCK Cafe'}</p>
          <p className="text-sm font-bold text-orange-600 mt-1">
            Amount: Rs. {Number(order.totalAmount).toLocaleString()}
          </p>
        </div>
      )}

      {/* Screenshot submitted, waiting */}
      {status === 'PENDING' && uploaded && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-center mb-3">
          ...
          {payStatus?.screenshotUrl && (
            <a href={payStatus.screenshotUrl} target="_blank" rel="noreferrer"
              className="text-xs text-blue-500 underline mt-1 inline-block">View screenshot</a>
          )}
        </div>
      )}

      {/* Upload area — show if not verified AND (no screenshot yet OR rejected) */}
      {status !== 'COMPLETED' && (!uploaded || status === 'FAILED') && (
        <div>
          <p className="text-xs font-semibold text-gray-600 mb-2">Upload payment screenshot:</p>
          <div
            onClick={() => fileRef.current?.click()}
            className={`border-2 border-dashed rounded-xl cursor-pointer mb-3 transition-colors ${
              preview ? 'border-orange-300 bg-orange-50' : 'border-gray-200 hover:border-orange-300 bg-gray-50'
            }`}
          >
            {preview ? (
              ...
              <div className="flex flex-col items-center py-6 text-gray-400 gap-2">
                <Upload size={24} />
                <p className="text-sm font-medium">Tap to select screenshot</p>
